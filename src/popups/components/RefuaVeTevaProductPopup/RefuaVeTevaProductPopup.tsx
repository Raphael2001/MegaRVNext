...
export default function RefuaVeTevaProductPopup(props: Props) {
	const { payload = {}, popupIndex } = props;
	const { dataItem } = payload;

	const powerLinkProducts = useAppSelector((store) => store.init.powerLinkProducts);
	const wooProducts = useAppSelector((store) => store.init.refuaVeTevaProducts);
	...
	function onSubmit(formPayload, onSuccess) {
		const payload = { ...formPayload, source: WOO_SOURCES.REFUA_VE_TEVA };
