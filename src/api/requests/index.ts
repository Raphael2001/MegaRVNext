...
const Api = (function () {
  ...
  async function addSMSTemplate(props: ApiProps = {}) {
    function onSuccess(res: ApiResponse) {
      Store.dispatch(addNewKey({ value: res.body, name: "smsTemplates" }));
      typeof props.onSuccess === "function" && props.onSuccess(res.body);
    }
    props.headers = await accessTokenHeaders();
    return ApiManager.addCall(
  ...
  async function updateSMSTemplate(props: ApiProps = {}) {
    function onSuccess(res: ApiResponse) {
      Store.dispatch(updateKey({ value: res.body, name: "smsTemplates" }));
      typeof props.onSuccess === "function" && props.onSuccess(res.body);
    }
    props.headers = await accessTokenHeaders();
    return ApiManager.addCall(props, API_METHODS.PUT, "smsTemplate", onSuccess);
  }

  async function deleteSMSTemplate(props: ApiProps = {}) {
    function onSuccess(res: ApiResponse) {
      Store.dispatch(deleteKeyById({ value: res.body, name: "smsTemplates" }));
      typeof props.onSuccess === "function" && props.onSuccess(res.body);
    }
    props.headers = await accessTokenHeaders();
    return ApiManager.addCall(
